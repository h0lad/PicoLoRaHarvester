...
static int neh7100_api_init(const struct neh7100_device *dev)
{
...
static int neh7100_api_read(const struct neh7100_device *dev, struct neh7100_sample *sample)
{
    const struct neh7100_cfg *cfg = dev->cfg;
    uint8_t reg[2];
    ...
    sample->range = reg[0] & 0x03U;
    sample->current_na = ((uint32_t)reg[1] * scale_q1_na[sample->range]) / 10U;

    return 0;
}

const struct neh7100_api neh7100_api = {
    .init = neh7100_api_init,
    .read = neh7100_api_read,
};
