...
static void fault_dump(const char *tag)
{
  ...
  uint32_t pc   = *(volatile uint32_t *)(__get_MSP() + 24U);
  char msg[] = "  CFSR=00000000 HFSR=00000000 PC=00000000\r\n";
  int i;

  msg[0] = tag[0];
  msg[1] = tag[1];
  for (i = 0; i < 8; i++) msg[7 + 7 - i]  = hex[(cfsr >> (4 * i)) & 0xF];
  for (i = 0; i < 8; i++) msg[21 + 7 - i] = hex[(hfsr >> (4 * i)) & 0xF];
  for (i = 0; i < 8; i++) msg[33 + 7 - i] = hex[(pc >> (4 * i)) & 0xF];
